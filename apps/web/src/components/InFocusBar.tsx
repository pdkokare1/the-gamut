...
export const InFocusBar: React.FC = () => {
  ...
  return (
    ...
    <div className="w-full bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b border-border sticky top-[3.5rem] z-30 flex items-center h-12 px-4 shadow-sm">
      ...
      <div className="flex-1 flex gap-2 overflow-x-auto no-scrollbar mask-gradient items-center">
        {isLoading ? (
          ...
          topics.map((item: any, index: number) => (
            <button 
              key={index} 
              className={cn(
                "whitespace-nowrap px-3 py-1 rounded-full text-[11px] font-medium transition-all",
